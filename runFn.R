...
burnin = 2000		# Burn-in
variantes  <- list(K=c(3,3,4,4), K.hmm.sv=c(3,3,4,4), useHMM=c(FALSE, TRUE, FALSE, TRUE))
for(variante in 1:4){

  # FDLM parameters:
  K = variantes[[1]][variante]			# Number of factors
  K.hmm.sv = variantes[[2]][variante]			# Number of factors for the common trend model
  useHMM = variantes[[3]][variante]			# Hidden Markov model (HMM), or common trend (CT) model? (CT in the paper)
