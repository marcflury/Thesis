...
mfdlmBeta = function(Y, Et, Gt, Wt, Model, tau, d, splineInfo){
  ...
  F = array(0, c(length(tau), nrow(Gt)))
  inds = 1:K
...
initHMM = function(dBeta, K.hmm.sv, useHMM = FALSE, u01 = 1, u00 = 1, u10 = 1, u11 = 1){
  ...
  if(useHMM){
    ...
    # still return S for simpler computations elsewhere
    list(psi=psi, S=S, gammaSlopes=gammaSlopes)
  }
}
#####################################################################################################
#####################################################################################################
...
sampleSlopes = function(dB.ck, dB.1k, psi.ck, St, sigma.t2){
  ...
  Xtemp = (St[-1] * dB.1k[-1] - psi.ck * St[-n] * dB.1k[-n])/sqrt(sigma.t2[-1])
...
# Gt: evolution coefficients array, (2*C*K) x (2*C*K) x T
# Wt: evoution error variance array, (C*K) x (C*K) x T (only one part)
# S: T x (C*K) matrix of states
# gammaSlopes: (C*K)-dimensional vector of slopes (w/ appropriate placement of zeros)
# psi: (C*K)-dimesional vector AR(1) coefficients
# sigma.t2: T-dimensional vector of error variances from the Beta-level for outcome c, factor k
