...
insampleP <- function(Index, Beta, ht, K, C, psi, gammaSlopes,
...
Ps = Ps, q10 = q10, q01 = q01, sT = S[Index, ]))
...
rollColSums <- function(x,window){
  oneColSums <- function(rownum,window, x){
    colSums(x[rownum:(rownum+window-1),])
  }
  ret <- lapply(1:(nrow(x)-window+1), oneColSums, window=window, x=x)
...
pred1 <- function(x,Beta=Beta, ht=ht, K=K, C=C, psi=psi,
  ...
  do.call("rbind", lapply(2:nrow(Beta), insampleP, Beta=Beta, ht=ht, K=K, C=C, psi=psi,
                          gammaSlopes=gammaSlopes, svMu=svMu, svPhi=svPhi, svSigma=svSigma,
                          Ps=Ps, S=S, q10=q10, q01=q01 )
  )
}
...
outsampleP <- function(Index, Beta, ht, K, C, psi, gammaSlopes,
                       svMu, svPhi, svSigma, Ps, S, q10, q01){

  wT <- Beta[Index, ] - psi * Beta[Index-1, ] -
    gammaSlopes * S[Index, ] * (Beta[Index, 1:4] - psi * Beta[Index-1, 1:4])
...
formatPlotData <- function(Index, List, names, colours){
  as.data.frame(List[[Index]]) %>%
    gather(Tenor, Percentile) %>%
    mutate(Tenor = as.numeric(gsub("[^0123456789]", "", as.character(Tenor)))) %>%
    mutate(ColTag = as.factor(Tenor %/% ((max(Tenor)+1)/colours))) %>%
    mutate(Name = names[Index])
}

colourHist <- function(datamats, names, breaks=seq(0,1,0.05), colours=6,
                       myscale="fixed"){
  Imax <- length(datamats)
  PlotData <- do.call("rbind", lapply(1:Imax, formatPlotData, List=datamats,
                                      names=names, colours=colours))

  ggplot(PlotData, aes(x=Percentile, fill=ColTag, group=ColTag))+
    geom_histogram(breaks=breaks)+
    geom_hline(yintercept=nrow(PlotData)/Imax/(length(breaks)-1)) +
    facet_grid(Name~., scale=myscale)
}
